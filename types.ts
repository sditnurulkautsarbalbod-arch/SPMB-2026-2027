...
export type NotificationType = 'info' | 'blocking';
...
export const INITIAL_NOTIFICATION: NotificationSettings = {
  isActive: false,
  type: 'info',
  title: '',
  message: ''
...
// Gelombang (Wave) helper
export const GELOMBANG_CUTOFF_DATE = '2026-02-07'; // Gelombang 1 deadline

export const getGelombang = (tanggalPendaftaran: string): string => {
  if (!tanggalPendaftaran) return '-';

  // Parse the date - handle both formats: YYYY-MM-DD and DD-MM-YYYY
  let dateStr = tanggalPendaftaran.split('T')[0]; // Remove time if present

  // Check if format is DD-MM-YYYY and convert to YYYY-MM-DD for comparison
  ...
  const cutoffDate = new Date(GELOMBANG_CUTOFF_DATE);

  if (isNaN(registrationDate.getTime())) return '-';

  return registrationDate <= cutoffDate ? 'Gelombang 1' : 'Gelombang 2';
};
